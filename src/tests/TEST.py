...
def main():
    print("--- Fahrdynamik Visualisierung (Tagesfahrplan & Wendezeiten) ---")
    ...
    dynamic_out_dir = os.path.join(OUTPUT_DIR, "3_speed_profile", selected_provider, selected_city, selected_bus)
    ...
    for csv_path in selected_files:
        ...
        try:
            ...
            # Sicherheits-Fallback für Strings mit Kommas
        ...
        layer_stops = folium.FeatureGroup(name="Haltestellen & Wendeanlagen", show=True)
        ...
        for i in range(len(df)):
            ...
            # Wenn der Bus steht UND es ein Halt/Wendeplatz ist
            if ('W' in typ or 'H' in typ) and not current_visit_recorded and speed < 0.1:
                ...
                # Sperre für diese Halte-Periode, damit er in Sekunde 2 nicht noch eine Zeile schreibt
        ...
        legend_html = '''
        <div style="position: fixed; bottom: 50px; left: 50px; width: 190px; height: 190px; border:2px solid grey; z-index:9999; font-size:12px; background-color:white; opacity:0.9; padding: 10px;">
