...
# =========================================================
# Hier versucht das Skript sich am Fahrplan bzw. den Zeiten des Fahrplans zu orientieren. Wenn der Bus der Zeit voraus ist, macht es langsamer und anders rum!
# 1. KONFIGURATION
# =========================================================
...
DWELL_TIME_STOP_SECONDS = 30.0  # Zwingende Mindest-Wartezeit an Stationen
...
def build_time_series(df, dists, cum_dist, v_spatial, road_limits_kmh, scenario_limits_ms, is_stop, file_name, v_ref_ms, a_ref):
    ...
    df['sec_ankunft'] = df.get('Ankunft', pd.Series(dtype=str)).apply(time_to_seconds)
    df['sec_abfahrt'] = df.get('Abfahrt', pd.Series(dtype=str)).apply(time_to_seconds)

    # Echte Startzeit der allerersten Haltestelle auslesen
    start_time_sec = 0.0
    for val in df['sec_abfahrt']:
        if pd.notna(val):
            start_time_sec = val
            break
    ...
    stop_indices = np.where(df['Type'] == 'W')[0]

    # --- Dynamische Puffer-, Haltezeit- und Verspätungslogik ---
    ...
    for i in range(len(stop_indices) - 1):
        ...
        orig_sched_dep = df['sec_abfahrt'].iloc[idx_start]
        orig_sched_arr = df['sec_ankunft'].iloc[idx_end]

        if pd.isna(orig_sched_dep) or pd.isna(orig_sched_arr):
            continue

        current_dep = orig_sched_dep + current_delay

        t_phys_drive = np.sum(baseline_dt[idx_start:idx_end])
        t_phys_total = t_phys_drive
        t_sched_budget = orig_sched_arr - current_dep
        if t_sched_budget < -43200: t_sched_budget += 86400  # Nacht-Übergang absichern

        if t_sched_budget > t_phys_total:
            # Bus hat Zeit -> Cruising (fährt langsamer)
            target_drive_time = t_sched_budget
            if target_drive_time <= 0: target_drive_time = 0.1

            v_factor = t_phys_drive / target_drive_time
            if v_factor > 1.0: v_factor = 1.0

            actual_v_spatial[idx_start:idx_end] *= v_factor
            actual_dt[idx_start:idx_end] /= v_factor
            actual_arr = orig_sched_arr
        else:
            # Bus ist zu spät -> Vollgas (physikalisches Maximum)
            actual_arr = current_dep + t_phys_total

        # Standzeit berechnen
        if i + 1 < len(stop_indices) - 1:
            orig_next_dep = df['sec_abfahrt'].iloc[idx_end]
            if pd.isna(orig_next_dep): orig_next_dep = orig_sched_arr
            if orig_next_dep < orig_sched_arr - 43200: orig_next_dep += 86400

            earliest_dep = actual_arr + DWELL_TIME_STOP_SECONDS
            actual_dep = max(orig_next_dep, earliest_dep)

            wait_times[idx_end] = actual_dep - actual_arr
            current_delay = actual_dep - orig_next_dep
        else:
            wait_times[idx_end] = DWELL_TIME_STOP_SECONDS

    # --- Erweiterte Timeline aufbauen ---
...
def main():
    print("\n" + "=" * 60)
    print(" SPEED PROFILE GENERATOR (Mit Verspätungs-Propagation & Fahrt_Nr)")
    print("=" * 60)
    ...
    # Extrahieren des Namens der Beschleunigungs-Datei ohne die Endung ".csv"
    bus_variant_prefix = os.path.splitext(os.path.basename(selected_acc_file))[0]

    # --- 2. SZENARIO (SPALTE) IN DER DATEI AUSWÄHLEN ---
    try:
        # Kurzer Vorab-Lesezugriff, um nur die Spaltennamen zu laden
    ...
    # Spalten, die keine Geschwindigkeits-Spalte sind, gelten als Szenarien
    ...
    if len(sys.argv) > 3:
        ...
        files = glob.glob(os.path.join(route_dir, "*Umlauf_*.csv"))
        if not files: return print(f"Keine '*Umlauf_*.csv' gefunden in {route_dir}!")
    ...
    for csv_path in selected_files:
        ...
        out_name = file_name.replace(".csv", f"_{bus_variant_prefix}_SpeedProfile_Factor{LIMIT_FACTOR}.csv")
