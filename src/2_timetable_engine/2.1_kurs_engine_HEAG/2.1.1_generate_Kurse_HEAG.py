...
def main():
    ...
    for f in route_files:
        ...
        try:
            ...
            df = pd.read_csv(f, sep=None, engine='python', encoding='cp1252', on_bad_lines='skip')

        if 'Type' not in df.columns or 'Name' not in df.columns:
            print(f"  -> [WARNUNG] Überspringe '{os.path.basename(f)}': Spalten 'Type' oder 'Name' fehlen!")
            continue
        # =================================================================
    ...
    for sheet_name in xls.sheet_names:
        ...
        # --- NEU: Berechnung der Gesamtzahl der Trips für die Anzeige ---
        valid_groups = [group for _, group in df_plan.groupby('Fahrt_Nr') if len(group) >= 2]
        total_valid_trips = len(valid_groups)
        matched_count = 0

        print(f"\n  Suche Geometrien für {total_valid_trips} extrahierte Trips im Bus '{sheet_name}'...")
        ...
        for trip_id, group in df_plan.groupby('Fahrt_Nr'):
            ...
            if df_route is not None:
                ...
                # Zeit der ersten Haltestelle auf den absolut ersten Punkt der Route übertragen!
                ...
                # --- NEU: Zähler bei Erfolg anzeigen (z.B. "1/22") ---
                print(
                    f"    + Trip {trip_id} ({matched_count}/{total_valid_trips}): '{group.iloc[0]['Name']}' -> '{group.iloc[-1]['Name']}'")
            else:
                missing_routes = True
                # --- NEU: Fette Warnung, welcher Trip exakt fehlt ---
                print(
                    f"    - [FEHLER] Trip {trip_id} konnte NICHT nachgebaut werden! Keine Geometrie gefunden für: '{group.iloc[0]['Name']}' -> '{group.iloc[-1]['Name']}'")

        # --- NEU: Zusammenfassende Ausgabe am Ende ---
        print(
            f"\n  => Zusammenfassung: {matched_count}/{total_valid_trips} Trips für '{sheet_name}' komplett nachgebaut.")
        ...
        if final_geometries:
            ...
            print(f"  => Datei erfolgreich gespeichert: {out_name}")

            if missing_routes:
                print(
                    f"  => ACHTUNG: Der Umlauf ist nicht vollständig! Es fehlen {total_valid_trips - matched_count} Fahrten in der Ausgabe.")
